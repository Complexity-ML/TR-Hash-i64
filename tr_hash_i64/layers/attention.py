"""
tr-hash-i64 :: Attention Backends
...
def _gpu_softmax_integer(scores: torch.Tensor) -> torch.Tensor:
    ...
    if scores.is_cuda:
        try:
            from tr_hash_i64.kernels.triton.I64_fused_softmax import triton_fused_softmax_integer
            ...
            pass
    from tr_hash_i64.layers.moe import softmax_integer
...
def naive_integer_varlen_attention(
    ...
    """
    from tr_hash_i64.layers.moe import softmax_integer
    from tr_hash_i64.core.quantization import quantize_activations_int8, _INT_MM_AVAILABLE
    ...
    for n in tokens_per_seq:
        ...
        # Always use float bmm for attention scores — INT8 Q@K^T quantization
        # is too aggressive and degrades output quality (especially for small models).
        # INT8 savings come from linear projections (QKV, O, MLP), not attention scores.
...
def naive_integer_cached_attention(
    ...
    # Always use float bmm for attention scores (see prefill comment above)
    scores = torch.bmm(q_t.float(), k_t.float().transpose(1, 2)) * softmax_scale
