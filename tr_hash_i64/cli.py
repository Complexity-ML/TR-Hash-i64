...
Usage:
    tr-hash-i64 serve <model> [--port 8000] [--host 0.0.0.0] [--dtype float16] [--tp 1]
    tr-hash-i64 bench [--mode all|routing|engine] [--requests 20] [--concurrency 8]
    tr-hash-i64 list
...
def cmd_serve(args):
    ...
    if getattr(args, 'no_model', False):
        from tr_hash_i64.core.logging import setup_logging
        setup_logging(level="INFO", json_output=getattr(args, 'log_json', False))
        from tr_hash_i64.api.server import I64Server

        _logger.info("tr-hash-i64 :: sandbox-only mode (no model loaded)")
        server = I64Server(
            engine=None,
            tokenizer=None,
            chat_template=None,
    ...
    if args.tp > 1 or args.pp > 1:
        ...
        if getattr(args, 'disaggregated', False) and args.tp >= 2:
            from tr_hash_i64.parallel.disaggregated import launch_disaggregated
        ...
        from tr_hash_i64.parallel.launcher import launch_distributed
    ...
    if device == "cpu":
        ...
        cpu_threads = int(
            os.environ.get("TR_HASH_I64_CPU_THREADS", available_cpus)
        )
    ...
    _logger.info("tr-hash-i64 :: serving %s", args.model)
...
def cmd_bench(args):
    """Run benchmarks."""
    import os
    # benchmarks/ lives at project root, not inside tr_hash_i64 package
...
def cmd_agent(args):
    """Run the agentic loop against a tr-hash-i64 server."""
    from tr_hash_i64.agentics import Agent
    ...
    if not agent.client.health():
        print(f"Error: cannot reach tr-hash-i64 server at {args.server}")
        print(f"Start it first: tr-hash-i64 serve pacific-prime-chat --checkpoint ... --port 8000")
        sys.exit(1)
...
def main():
    ...
    # agent
    p_agent = sub.add_parser("agent", help="Run AI agent (connects to a running tr-hash-i64 server)")
    ...
    p_agent.add_argument("--server", default="http://localhost:8000",
                         help="tr-hash-i64 server URL (default: http://localhost:8000)")
